...
class MainMenuState extends MusicBeatState
{
	...
	#if !switch
	var optionShit:Array<String> = ['story mode', 'freeplay', 'secret', 'options'];
	#else
	var optionShit:Array<String> = ['story mode', 'freeplay', 'secret'];
	#end
	...
	var bgch:FlxSprite;
	var diff = 1;
	var infoTxt:Array<String> = ['EASY', 'GOD'];
	var info:FlxText;
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				if (optionShit[curSelected] == 'secret')
				{
					var athanatos:String = Highscore.formatSong('athanatos', 1);

										trace(athanatos);
							
										PlayState.SONG = Song.loadFromJson(athanatos, 'athanatos');
